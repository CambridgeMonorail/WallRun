...
- that generated `all.json` item must list each published registry item by its fully qualified WallRun URL (for example, `https://wallrun.dev/registry/<item-name>.json`) so the shadcn CLI does not fall back to the default `ui.shadcn.com` registry during recursive installs
- when published registry items change, regenerate and check in `apps/client/public/registry/all.json` so the repository artifact matches this URL-based contract
...
- `registryDependencies` is effectively unused by published component items today. The only populated case is the generated `all.json` meta item, which intentionally lists every published registry item by fully qualified WallRun URL.
