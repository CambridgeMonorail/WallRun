...
## BrightSign Model Quick Reference
...
- Use `signage-animation-system` and follow its `will-change` guidance to stay within GPU budgets.
- Use `signage-distance-legibility` to right-size assets — oversized images waste budget.
- Use `brightsign-runtime` for detailed runtime constraints beyond performance budgets.
